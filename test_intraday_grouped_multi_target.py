"""
Test group-based multi-target prediction for IntradayPredictor.

This test verifies that multi-target prediction works correctly when using
group-based scaling (e.g., different symbols get different scalers).
"""
...
# Create synthetic multi-symbol intraday data
np.random.seed(42)
...
for symbol in symbols:
    # Each symbol has different price ranges
    base_price = 50000 if symbol == 'BTC' else 3000
    base_volume = 100000 if symbol == 'BTC' else 50000

    for i in range(200):
        timestamp = pd.Timestamp('2024-01-01') + pd.Timedelta(minutes=i)
        price_noise = np.random.randn() * 1000
        volume_noise = np.random.randn() * 10000

        data.append({
            'timestamp': timestamp,
            'symbol': symbol,
            'open': base_price + price_noise,
            'high': base_price + price_noise + abs(np.random.randn() * 500),
            'low': base_price + price_noise - abs(np.random.randn() * 500),
            'close': base_price + price_noise + np.random.randn() * 200,
            'volume': max(1, base_volume + volume_noise)
        })

df = pd.DataFrame(data)

print("=" * 80)
print("Test: Group-Based Multi-Target Prediction for IntradayPredictor")
print("=" * 80)
...
print("\n" + "=" * 80)
print("Test 2: Multi-Target ['close', 'volume'], Multi-Horizon (3), Group-Based Scaling")
print("=" * 80)
...
assert predictions['close'].shape[1] == 3, "Should have 3 horizons for close"
assert predictions['volume'].shape[1] == 3, "Should have 3 horizons for volume"
    ...
    if symbol in model2.group_target_scalers:
        ...
        for target in ['close', 'volume']:
            if target in scalers:
                ...
                assert len(scalers[target]) == 3, f"{target} should have 3 horizon scalers"
...
for symbol in symbols:
    ...
    if symbol_count > 0:
        ...
        print(f"  Volume range: [{volume_preds.min():.2f}, {volume_preds.max():.2f}]")

        # Verify different symbols have different prediction ranges
        # (since they have different base prices/volumes)
...
print("=" * 80)
print("\nGroup-based multi-target prediction is working correctly for IntradayPredictor:")
